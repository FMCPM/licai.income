...
//	{
//		[_touchesdelegate paginatorViewTouchesEnd:touches withEvent:event whichView:self];
//	}
//	NSLog(@"UITouchScrollView nextResponds touchesEnded");
//	}
...
-(void)initializeWithFrame:(CGRect)frame
{
    ...
    _pageControl = [[SYPageControl alloc] initWithFrame:CGRectMake(0.0f, size.height -15, size.width, 15.0f)];
...
- (void)setTopPointShow
{
    CGSize size = self.bounds.size;
    _pageControl.frame = CGRectMake(0.0f, 0, size.width, 15.0f);
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
	   ...
	   if(!_delegate)
		...
		if([_delegate respondsToSelector:@selector(actionPageViewClicked)])
	 	[_delegate actionPageViewClicked];

}
//-(void)scrollViewTouchesEndedView:(NSSet *)touches withEvent:(UIEvent *)event whichView:(id)scrollVuew
//{
	//NSLog(@"123123");
//	AhWebView *webView = [[AhWebView alloc]init];
//	[]
//}
//-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
//{
