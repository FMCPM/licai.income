...
@synthesize waterFlowViewDelegate = _waterFlowViewDelegate;
@synthesize waterFlowViewDatasource = _waterFlowViewDatasource;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (cell == nil) {
        ...
        IndexPath *_indextPath = [IndexPath initWithRow:indexPath.row  withColumn:tableView.tag - TABLEVIEWTAG];
        WaterFlowViewCell *cellSub =  [self.waterFlowViewDatasource waterFlowView:self cellForRowAtIndexPath:_indextPath];
        cellSub.tag = CELLSUBVIEWTAG;
        cellSub.columnCount = self.columnCount;
        [cell.contentView addSubview:cellSub];
    }
    
    IndexPath *_indextPath = [IndexPath initWithRow:indexPath.row  withColumn:tableView.tag - TABLEVIEWTAG];
    ((WaterFlowViewCell*)[cell viewWithTag:CELLSUBVIEWTAG]).indexPath = _indextPath;
    CGFloat cellHeight = [self.waterFlowViewDelegate waterFlowView:self heightForRowAtIndexPath:_indextPath];
    CGRect cellRect = CGRectMake(0, 0, _cellWidth, cellHeight);
    [[cell viewWithTag:CELLSUBVIEWTAG] setFrame:cellRect];
    [self.waterFlowViewDatasource waterFlowView:self relayoutCellSubview:(WaterFlowViewCell*)[cell viewWithTag:CELLSUBVIEWTAG] withIndexPath:_indextPath];
    [(WaterFlowViewCell*)[cell viewWithTag:CELLSUBVIEWTAG] relayoutViews];
