...
char pinyinSecondLetter(unsigned short hanzi) {
	...
	if( !string || 0 == [string length] )
		...
		if(([string characterAtIndex:0] > 64 && [string characterAtIndex:0] < 91) || 
		   ([string characterAtIndex:0] > 96 && [string characterAtIndex:0] < 123) ) {
			...
			cLetter = pinyinSecondLetter((unsigned short)[string characterAtIndex:0]);
...
+ (NSString *) firstCharFromChiniseString:(NSString *)string
{
	NSString *pinYinResult = [NSString string];
	if (![string isEqualToString:@" "]) {
		for (int i=0; i<string.length; i++) {
			NSString*single = [[NSString stringWithFormat:@"%c",pinyinSecondLetter([string characterAtIndex:i])]uppercaseString];
			pinYinResult = [pinYinResult stringByAppendingString:single];
		}
	}
	else
	{
