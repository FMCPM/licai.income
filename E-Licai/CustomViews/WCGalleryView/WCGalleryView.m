...
@synthesize stackRadiusDirection = _stackRadiusDirection;
@synthesize stackRadiusOffset = _stackRadiusOffset;
...
//        self.gestureRecognizers = @[pinchGesture];
    ...
    self.shadowOffset        = CGSizeMake(1.0f, 2.0f);
    self.shadowOpacity       = 0.5f;
    self.shadowRadius        = 2.0f;
    self.stackRadiusOffset   = 9.0f;
    self.animationDuration   = 0.3f;
    self.stackRadiusDirection = WCGalleryStackRadiusRandom;
    self.animationType        = WCGalleryAnimationFade;
    ...
    [self insertImageViewAtIndex:0 animated:NO];
    return self;
}
...
- (void)addImage:(UIImage *)image animated:(BOOL)animated
{
    ...
    [_imageQueue addOperationWithBlock:^{
        [self.imageViews insertObject:image atIndex:0];

        [super performSelectorOnMainThread:@selector(runLoadImage:) withObject:self.imageViews waitUntilDone:YES];
    }];
...
-(void)startAnimation
{
    if(!_haveBuiltImageViews)
    {
        for(NSInteger i=1; i < _images.count; i++)
        {
        ...
        for(NSInteger total = 1; total < _imageViews.count; total++)
            [self insertImageViewAtIndex:total animated:_animate];
...
{
    if (_delegate) {
        if([_delegate respondsToSelector:@selector(wcGalleryViewClick:)])
        {
            [_delegate wcGalleryViewClick:self];
        }
    }
    /*
