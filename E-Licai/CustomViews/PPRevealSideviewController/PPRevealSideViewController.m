...
@synthesize oldPanInteractionsWhenClosed = _oldPanInteractionsWhenClosed;
...
- (id) initWithRootViewController:(UIViewController*)rootViewController {
    ...
    if (self) {
        ...
        self.panInteractionsWhenClosed = PPRevealSideInteractionNavigationBar | PPRevealSideInteractionContentView;
        _oldPanInteractionsWhenClosed = self.panInteractionsWhenClosed;
...
- (void)loadView
{
    ...
    //modify by lzq at 2013-06-26,主要是去掉窗口下面的tab bar的黑色条
...
- (void) setPanInteractionsWhenClosed:(PPRevealSideInteractions)panInteractionsWhenClosed {
    ...
    _oldPanInteractionsWhenClosed = _panInteractionsWhenClosed;
...
- (void) addPanGestureToController:(UIViewController*)controller {
    ...
    if (interactions & PPRevealSideInteractionNavigationBar && ([controller isKindOfClass:[UINavigationController class]] || controller.navigationController || [controller isKindOfClass:[UITabBarController class]])) {
        ...
        if ([controller isKindOfClass:[UITabBarController class]]) {
            for (UINavigationController *ctrl in [(UITabBarController*)controller viewControllers]) {
                if ([ctrl isKindOfClass:[UINavigationController class]]) {
                    ...
                    pan.delegate = self;
                    if (!isClosed) {
                        UIView *coverView = [[UIView alloc]initWithFrame:nav.navigationBar.bounds];
                        coverView.backgroundColor = [UIColor clearColor];
                        coverView.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
                        [ctrl.navigationBar addSubview:coverView];
                        [_coverViews addObject:coverView];
                    }
                    [ctrl.navigationBar addGestureRecognizer:pan];
                    [_gestures addObject:pan];
                    PP_RELEASE(pan);
                }
            }
        }
        ...
        if (!isClosed && nav) {
            ...
            coverView.backgroundColor = [UIColor clearColor];
            coverView.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
            [nav.navigationBar addSubview:coverView];
            [_coverViews addObject:coverView];
        }
        if (nav) {
    ...
    if (interactions & PPRevealSideInteractionContentView) {
        ...
        if ([controller isKindOfClass:[UINavigationController class]]) {
            ...
            if (controller.navigationController)
                ...
                c = controller;
        if (!isClosed) {
            UIView *coverView = [[UIView alloc]initWithFrame:c.view.bounds];
            coverView.backgroundColor = [UIColor clearColor];
            coverView.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
            [c.view addSubview:coverView];
            [_coverViews addObject:coverView];
        }
...
- (void) removeAllGestures {
    for (UIView *coverView in _coverViews) {
