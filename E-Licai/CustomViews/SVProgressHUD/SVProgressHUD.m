...
+ (void)showWithStatus:(NSString *)status duration:(NSTimeInterval)duration {
    [[SVProgressHUD sharedView] showWithStatus:status maskType:SVProgressHUDMaskTypeNone networkIndicator:YES];
    [SVProgressHUD dismissAfterDelay:duration];
}
...
+ (void)dismissAfterDelay:(NSTimeInterval)seconds {
    [[SVProgressHUD sharedView] performSelector:@selector(dismiss) withObject:nil afterDelay:seconds];
}
...
{
    return [[SVProgressHUD sharedView].spinnerView isAnimating];
}
